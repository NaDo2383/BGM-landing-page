    ...
    <section className='bg-[#020618] p-6 sm:p-10 lg:p-[100px]'>
      <div className='mx-auto max-w-[1240px] px-2 sm:px-4 flex flex-col gap-8 sm:gap-10 lg:gap-[50px]'>
        ...
        <div className='flex flex-col gap-2'>
          ...
          <p className='text-center text-[#90A1B9] font-normal tracking-normal capitalize text-sm sm:text-base leading-relaxed max-w-3xl mx-auto'>
            {t("text")}
          </p>
        </div>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-[50px]'>
          {capabilities.map((cap, i) => (
            <div key={i} className='h-full'>
              {/* Wrap to enforce equal heights regardless of content lengths */}
              <Card {...cap} />
            </div>
          ))}
        </div>
